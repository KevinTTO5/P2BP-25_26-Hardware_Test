#!/usr/bin/env bash
# installer/mv3dt_installer/assets/scripts/10_setup_mosquitto.sh
#
# Notion page 337b5d58-7212-81e1-b07a-d510d9605bbb §6 — Mosquitto install &
# drop-in configuration, ported from laptop/scripts/10_setup_mosquitto.sh
# (no installer step owns Mosquitto install; see
# installer/plan/DELETION-REVIEW.md §6).
#
...
#   2. Install the bundled mv3dt.conf into /etc/mosquitto/conf.d/mv3dt.conf
...
#
# This script must be run via mv3dt_installer.shellout.run_bundled_script
# with tree=() (the whole assets/ tree staged), so that assets/mosquitto/
# lands alongside assets/scripts/ and $(asset_root)/mosquitto/mv3dt.conf
# resolves. It never reaches outside the staged asset tree except to write
# under /etc/ and call `apt-get` / `systemctl` / `ufw`.
#
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EOF
  if [[ "${NONINTERACTIVE:-0}" -eq 0 && "${MV3DT_NO_PAUSE:-0}" -eq 0 ]]; then
...
Installs mosquitto packages if needed, drops the bundled mv3dt.conf into
...
SRC_CONF="$(asset_root)/mosquitto/mv3dt.conf"
DST_DIR="/etc/mosquitto/conf.d"
DST_CONF="$DST_DIR/mv3dt.conf"

if [[ ! -f "$SRC_CONF" ]]; then
  die "Missing source config: $SRC_CONF (expected in the staged asset tree)."
fi
...
- MQTT_HOST, MQTT_PORT, MQTT_TOPIC_BASE in installer.conf must match what the pipeline step passes to the DeepStream nvmsgbroker sink"
