#!/usr/bin/env bash
# laptop/scripts/60_record_tracking.sh
#
...
#
# Output root (gitignored by laptop/.gitignore update):
#   laptop/tracking_exports/<YYYYMMDD_HHMMSS>_<location_id>/
#
# Designed to run in a second terminal while 50_start_pipeline.sh is active.
...
Subscribes to MQTT track topics and writes local exports under:
  laptop/tracking_exports/<run_id>/

Options:
  --duration SEC    Stop automatically after SEC seconds (default: run until Ctrl-C).
  --run-name NAME   Custom run suffix; default is LOCATION_ID from laptop.env.
  --output-root DIR Override output root directory (default: laptop/tracking_exports).
...
if [[ -n "$OUTPUT_ROOT_OVERRIDE" ]]; then
  ...
  OUTPUT_ROOT="$(repo_root)/laptop/tracking_exports"
fi
